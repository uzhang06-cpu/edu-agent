...
- web_search 的 query 应精炼，去掉"请帮我查"等口语` },
    ...
    return {
      ...
      updateSummary: async () => {
        try {
          const prompt = `你在维护一段"客户档案式"的对话记忆。请在【原摘要】基础上**增量更新**，输出更新后的完整摘要。
...
更新规则（重要）：
1. 累积记忆，不要丢弃原摘要里的已知事实，除非被新信息推翻
2. 必须牢牢保留这些关键事实（有就写）：
   - 孩子姓名 / 称呼、年级、薄弱学科
   - 家长诉求、预算倾向
   - 已报价的课程与价格
   - ✅ 已达成的约定（如"已约今晚7点试听""已加微信"）——这类务必保留，避免重复确认
   - 用户已明确拒绝或已确认的事项
3. 用简短要点式，一句一个事实，控制在 150 字内
4. 只输出摘要正文，不要解释
...
- 核心诉求：${(perception.key_intents || []).join('；') || '按上下文自行判断'}`);
  ...
  sections.push(`【输出约束 —— 像真人客服，不像机器人】
1. 直接输出回复正文，不要"(思考过程)"、"【动作描述】"、"（旁白）"等标记，不要"AI:/Bot:/助手:"前缀
2. 拒绝 AI 味：不说"作为 AI 助手""很高兴为您服务""祝您…愉快~"这类套话
3. 【严禁重复】不要重申本轮对话里已经说过的信息、时间或承诺。已约定好的事（如"今晚7点试听"）不要反复确认；用户已经答应/知晓的，别再解释一遍
4. 【读懂确认语】用户只回"可以/好的/哦哦/嗯"这类简短确认时，用一句话轻应即可（如"好，到时见"），不要再展开介绍或推销
5. 【别每句都推销】不要每条都用"有任何问题随时找我""要不要先试一节"收尾。只有当用户提出新疑虑或新问题时，才顺势给一次建议/引导
6. 简明扼要：能一句说清绝不两句；除非用户明确要详细`);
